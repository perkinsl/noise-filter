...
#Tests 500 random epsilon, delta pairs and returns a probability matrix for each one
...
def infer_models(data, epsilon, delta, gammas):
	models = []
...
## randomly initializes an (epsilon, delta) pair and infers model probabilities for that pair	
def test_epsilon_delta(data, iterations):
	...
	for i in range(0, iterations):
		...
		print('iteration', i)
		#sample random value of epsilon and delta
		...
		#infer models
		models = infer_models(data, epsilon, delta, gammas)
		print('models', models)
		
		
		modeltable.append(list(itertools.chain([epsilon], [delta], models)))
	
	print(modeltable)
	np.savetxt('test_epsilon_delta_modeltable', modeltable, fmt = '%1.2f')	
...
print(test_epsilon_delta(data, 500))