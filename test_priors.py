...
#Returns MAP estimate for each verb in data:
...
#Tests 500 random settings of the model's prior over M1, M2, and M3, with epsilon and delta fixed to 0.20 and 0.23
...
def infer_models(data, epsilon, delta, gammas, M1prior, M3prior):
	models = []
	...
	for i in range(0, len(data)):
		...
		## append MAP estimate of model for each verb
		modelprobs = [M1, M2, M3]
		if M1 == max(modelprobs):
			models.append(1)
		
		elif M2 == max(modelprobs):
			models.append(2)
...
## randomly initializes a P(M1), P(M3) pair and infers model probabilities for that pair	
def test_priors(data, iterations):
	...
	for i in range(0, iterations):
		...
		print('iteration', i)
		#sample random value of P(M1) and P(M3)
		M3prior = random.random()
		M1prior = random.uniform(0, (1.0-M3prior))
		print('M1', M1prior)
		print('M3', M3prior)
		
		#infer models
		models = infer_models(data, 0.20, 0.23, gammas, M1prior, M3prior)
		print('models', models)
		
		
		modeltable.append(list(itertools.chain([M1prior], [M3prior], models)))
	
	print(modeltable)
	np.savetxt('test_priors_modeltable', modeltable, fmt = '%1.2f')	
